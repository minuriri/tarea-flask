...
def agregar():
    if request.method == 'POST':
        if not request.form['palabra'] or not request.form['significado']:
            flash('Ingrese los datos correctamente', 'error')
        else:
...
def editar():
    if request.method == 'POST':
        if not request.form['palabra'] or not request.form['significado']:
            flash('Ingrese los datos corretamente', 'error')
        else:
...
def buscar():
    global result
    if request.method == 'POST':
        result = db.session.query(diccionario).filter_by(palabra=request.form['palabra']).first()
        return redirect(url_for('resultado'))
    return render_template("buscar.html")

@app.route("/resultado")
def resultado():
    return render_template("resultado.html", palabras=result)
